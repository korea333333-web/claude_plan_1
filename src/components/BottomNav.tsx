...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-bg-deep border-t border-border-subtle pb-[env(safe-area-inset-bottom,8px)]">
      <div className="flex justify-around items-center pt-3 pb-1 max-w-lg mx-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[13px] tracking-[0.5px] ${isActive ? 'text-accent-gold font-semibold' : ''}`}>
                {item.label}
              </span>
            </Link>
          );
      ...
      <div className="w-[134px] h-[5px] bg-white/20 rounded-full mx-auto mt-1.5" />
    </nav>
  );
}
